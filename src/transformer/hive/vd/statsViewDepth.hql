...
-- 5. 上传transformer-0.0.1.jar到hdfs的/transformer文件夹中
-- 6. 创建hive的function
#create function platform_convert as 'transformer.hive.PlatformDimensionUDF' using jar 'hdfs://sxt/sxt/transformer/transformer-0.0.1.jar';  
create function date_convert as 'transformer.hive.DateDimensionUDF' using jar 'hdfs://sxt/sxt/transformer/transformer-0.0.1.jar';  
...
from event_logs 
where en='e_pv' and p_url is not null and pl is not null and s_time >= unix_timestamp('2020-06-08','yyyy-MM-dd')*1000 and s_time < unix_timestamp('2020-06-09','yyyy-MM-dd')*1000
...
-- 9. sqoop脚本编写(统计会话角度)
sqoop export --connect jdbc:mysql://node3:3306/report --username root --password *********** --table stats_view_depth --export-dir /hive/bigdater.db/stats_view_depth/* --input-fields-terminated-by "\\01" --update-mode allowinsert --update-key platform_dimension_id,data_dimension_id,kpi_dimension_id
